...
name: khazix-writer
description: |
  数字生命卡兹克（Khazix）的公众号长文写作与排版skill。当用户需要撰写公众号文章、写稿子、续写文章、根据素材产出长文时使用。触发词包括但不限于：写文章、写稿子、帮我写、续写、扩写、公众号文章、长文、出稿、按我的风格写。即使用户只是说"帮我把这个写成文章"或"用我的风格写一下"，只要上下文涉及内容创作和公众号输出，都应该触发。也适用于用户丢过来一个PDF、brief、新闻链接、语音转文字或任何素材说"帮我写篇文章"的场景。当用户需要把已成稿的文章排成卡兹克公众号版式、生成能直接粘贴到公众号编辑器的HTML时，也用本skill（触发词：排版、排个版、生成公众号HTML、排成卡兹克格式），支持写完直接接排版的一条龙流程。本skill只负责产出文字与排版HTML，不负责把文章群发或发布到公众号（那类需求用发布类工具）。不要用于短内容（小红书帖子、推特、朋友圈）或纯标题摘要生成（那个用wechat-title skill）。
---
...
本步的分工来自skill合并的历史：写作部分（第一~四步）只管文字，排版部分只管版式。两层冲突时一律文字层赢，具体落成下面三条铁律。

### 排版三铁律

**1. 排版不写作。** 进入排版的内容必须是四层自检通过的终稿。排版过程禁止改写句子、禁止增删段落。允许的操作只有三类：从文中提取既有句子用作导语或金句块、给关键词加橙色高亮、把正稿的「图片」占位标记转成插图占位块。排版完成后，把HTML里的正文与自检通过稿逐段比对，除上述白名单模块外必须逐字一致。

**2. 声音优先于导航。** 流式文章不加小标题，导航交给分隔线（只在结构板块边界插，全文最多4条）。编号小节标题（01/02样式）只在两种情况启用：正稿本身已有"1、短语"式分条（方法论文章，短语直接样式化为节标题，不新写）；用户明确要求。外来文章只要求排版时，直接排版、不动文风；要求"改写+排版"才从第一步走起。

**3. 装饰服务停顿，不侵占文字。** 手机端横向空间全部让给正文：两侧不留框线，四周边框一律不用，只允许左缘细色条和横向装饰线。总量上限：分隔线≤4条、金句块≤2个、引用块≤2个、橙色高亮每千字≤2处。超限时删装饰，不删文字。

### 执行与交付

配色方案、微信粘贴兼容性铁律、完整HTML结构模板，严格按 `references/wechat_typography.md` 执行，不得即兴发挥。核心要求：全部样式内联、容器用section、background-color长写、禁渐变/box-shadow/flexbox/class、单层暖白纸底。成品保存为工作目录下的 `YYYYMMDD-标题.html`，随文件附上粘贴指引（复制渲染页面而非源码，兜底方案为135编辑器/秀米），并提醒公众号编辑器另有标题栏、正文大标题模块可按需删除。

## 参考资料

更详细的风格示例和修改对比，参考 `references/style_examples.md`。
完整的内容方法论（选题来源、选题分类、过往爆款案例、创意案例工作法），参考 `references/content_methodology.md`。
微信公众号版式规范（配色、粘贴兼容铁律、HTML结构模板、交付说明），参考 `references/wechat_typography.md`。
